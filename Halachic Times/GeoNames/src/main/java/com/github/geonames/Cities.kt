...
open class Cities {
    ...
    fun filterCapitals(names: Collection<GeoNamesToponym>): Collection<GeoNamesToponym> {
        ...
        if (!countries.isEmpty()) {
            val best: SortedMap<String, GeoNamesToponym> = TreeMap()
            var place: GeoNamesToponym?
            var cc: String
            for (name in names) {
                cc = name.countryCode ?: continue
    ...
    private fun betterPlace(name1: GeoNamesToponym, name2: GeoNamesToponym): GeoNamesToponym {
        ...
        // Compare populations.
        val pop1 = name1.population ?: 0
        val pop2 = name2.population ?: 0
    ...
    private fun getFeatureCodeRank(code: String?): Int {
        if (code.isNullOrEmpty()) return 0
        if (ranks.isEmpty()) {
            var rank = -2
    ...
    companion object {
        ...
        const val S_KEY_INDENT_AMOUNT = "{http://xml.apache.org/xalan}indent-amount"
